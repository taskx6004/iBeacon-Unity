...
void InitGLES(int api)
{
    ...
    {
        _supportsDiscard = _supportsMSAA = _supportsPackedStencil = false;
		...
		if(surface->msaaSamples > 1 && _supportsMSAA)
		{
			...
			if(surface->context.API < 3)
			{
				...
				if(_supportsDiscard)
				{
					...
					GLES_CHK(glDiscardFramebufferEXT(GL_READ_FRAMEBUFFER, 2, discardAttach));
				}
#endif
			...
			else
			{
				...
				GLenum	discardAttach[] = {GL_DEPTH_ATTACHMENT, GL_STENCIL_ATTACHMENT};
				GLES_CHK(glInvalidateFramebuffer(GL_READ_FRAMEBUFFER, 2, discardAttach));
				
				GLES_CHK(glBlitFramebuffer(0 ,0, surface->targetW, surface->targetH,
										   0, 0, surface->targetW, surface->targetH,
